...
(let ((default-linefeed (format nil "~C[0m~%" #\Esc))
  ...
  (defun write-text (terminal-buffer &key (stream *standard-output*) xmin ymin xmax ymax lineprefix linefeed)
    (declare (optimize (speed 3) (safety 0))
    ...
    (macrolet ((write-terminal-color (type stream red green blue 
      ...
      (with-terminal-buffer-size (tb-size-x tb-size-y) terminal-buffer
        (let ((xmin (or xmin 0)) (ymin (or ymin 0)) 
              ...
              (lineprefix (or lineprefix "")) (linefeed (or linefeed default-linefeed))
