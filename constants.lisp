         ...
         ,(let* ((cell-characters 
            ...
            (labels ((find-subchars (designator lst collected pixels)
                       (if (null lst)
                         ...
                         (progn
                           (find-subchars designator (rest lst) collected pixels)
                           (when (and (not (eql designator 
                             ...
                             (find-subchars designator (rest lst) 
                                            (cons (character-designator (first lst))
                                                  collected)
                                            (set-difference pixels (character-parts (first lst))
                                                            :test #'eql)))))))
              (let ((chars (copy-tree cell-characters)))
                (dolist (chr chars)
                  (find-subchars (character-designator chr) chars () (character-parts chr)))))
            cell-characters)) 
       :test #'equal)))
